...
module.exports.updateUserStats = asyncHandler(async (userId) => {
    ...
    let [{ sum: minutesSpent, count: readingsCompleted }] = await sequelize.query(selectQuerySumAndCount, { type: QueryTypes.SELECT });
    ...
    let [{ sum: pagesRead }] = await sequelize.query(selectQuerySumOfMaxes, { type: QueryTypes.SELECT });
    // const readingsCompleted = sessions.length;
    // const pagesRead = sessions.reduce((accumulator, session) => accumulator + session.minutes, 0);
    // const minutesSpent = 
    minutesSpent = minutesSpent === null ? 0 : minutesSpent;
    readingsCompleted = readingsCompleted === null ? 0 : readingsCompleted;
